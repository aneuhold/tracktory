...
```typescript
// Add more OAuth providers to Auth.js config
...
func (s *AuthService) EnableSSO(domain string, provider string) error {
    switch provider {
    case "saml":
        return s.sso.SetupSAML(domain)
    case "oidc":
        return s.sso.SetupOIDC(domain)
    default:
